...
for DS_no in range(10):
    entrainment_term = np.load(f'data_zeta_components_hcv250/entrainment_term_DS{DS_no}_30.npy')
    zeta = np.load(f'data/zeta_DS{DS_no}_30.npy')
    beta = np.load(f'data/beta_DS{DS_no}_30.npy')
    tauw0 = np.load(f'data/tauw0_DS{DS_no}_30.npy')
    cf0 = np.load(f'data/cf0_DS{DS_no}_30.npy')
    tautop0 = np.load(f'data/tautop0_DS{DS_no}_30.npy')
    tautop = np.load(f'data/tautop_DS{DS_no}_30.npy')
    M = np.load(f'data/M_DS{DS_no}_30.npy')
    model = (1/cf0 * (1e3) * (1+beta) ) / (30e3 * (1 - tautop0/tauw0))
    stress_ratio = tautop0/tauw0
    plt.scatter(M, tautop/tautop0, c=beta, vmin=0.85, vmax=0.95)

cbar = plt.colorbar()
cbar.set_label(r'$\tau_{top,0}/\tau_{w,0}$')
plt.ylabel(r'$\tau_{top}$')
plt.xlabel(r'$\tau_{top,0}$')
plt.ylim([0,4])
plt.xlim([0,4])
